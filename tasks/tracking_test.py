...
class TrackAltitudeEdit(TrackingTaskCascaded):
    """
    Track Altitude Edit: this is my edited version for tracking alt 
    """

    def __str__(self):
        return "tracking_altitude_edit"
    ...
    def set_tracking_ref(self):
        ...
        if self.evaluate_disturbance:
            ...
            h_0 = 2000
            self.tracking_ref["h"] = h_0 + 1000 * self.step(5, 2)
            self.tracking_ref["h"] -= 1000 * self.step(7, 2)
        ...
        if self.evaluate_disturbance:
           ...
           # self.tracking_ref["phi"] = A_phi * self.cosstep(self.T * 0.10 * 0.5, 1)
           # self.tracking_ref["phi"] -= A_phi * self.cosstep(self.T * 0.35 * 0.5, 1)
           # self.tracking_ref["phi"] -= A_phi * self.cosstep(self.T * 0.60 * 0.5, 1)
           # self.tracking_ref["phi"] += A_phi * self.cosstep(self.T * 0.85 * 0.5, 1)
           ...
           # self.tracking_ref["phi"] += A_phi * self.cosstep(self.T * 0.85 * 0.5 + 60, 2)
            ...
            self.tracking_ref["phi"] = A_phi * self.cosstep(2, 0.5)
            self.tracking_ref["phi"] -= A_phi * self.cosstep(2.5, 0.5)
            self.tracking_ref["phi"] += A_phi * self.cosstep(10, 1)
            self.tracking_ref["phi"] -= A_phi * self.cosstep(11, 1)
